<script lang="ts">
	...
	const { onResume, onNewGame }: Props = $props();
...
<svelte:window
	onkeydown={(event) => {
		...
		if (event.key == 'ArrowUp') {
			event.preventDefault();
			selectionIndex = (3 + selectionIndex - 1) % 3;
			return;
		}
...
>
	<h2 class="text-2xl">Game Paused</h2>
	<hr class="w-full" />
	<ul bind:this={linksParentEl}>
		...
		<li>
          ...
          relative
          focus:outline-none
          {selectionIndex == 2 ? 'text-primary-800 dark:text-primary-500' : ''}
          {selected == 2 ? 'selected' : ''}
          selection-link
          ps-4
          {selectionIndex == 2 ? 'before:opacity-100' : 'before:opacity-0'}
        "
				href="/"
				onfocus={() => {
					selectionIndex = 2;
